...
bool ImGui::SelectableSkin(const char* label, const ImVec4& colour, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (hovered || selected)
    {
        ...
        RenderFrame(bb.Min, bb.Max,ImGui::GetColorU32(colour), false, 0.0f);
    }
    if (g.NavId == id)
        RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
