...
namespace R2
{
    ...
    {
        glm::vec3 cameraPos = glm::vec3(0.0f, 13.0f, 5.0f);
        glm::vec3 cameraTarget = glm::vec3(0.0f, .0f, 0.0f); // or a direction vector
        glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);

        render_camera(){
            cameraPos = glm::vec3(0.0f, 2.0f, 5.0f);  // Position the camera at (0, 3, -10)
            cameraTarget = glm::vec3(0.0f, 1.0f, 1.0f); // Target a point directly ahead on the wall
    ...
    inline space get_world(){
      ...
      segs.push_back(segment({-21,0,5}, {-21,0,-21}, {21,0,-21}, {21,0,5}, {255, 0, 0, 255})); // Floor, CW order
        ...
        segs.push_back(segment({0,0,0}, {0,4,0}, {2,4,0}, {2,0,0}, {255,0,255,255}));
    ...
    */
    static void render_frame(CRenderer* renderer)
    {
       ...
       // rd.view.camera.cameraPos = {0,1,8};
      // rd.view.camera.cameraTarget = {0,1,1};
        ...
        for(auto& seg : scene.walls){
            ...
            for(int i = 0; i < seg.pt.size(); ++i){
              ...
              //  renderer->log(" w2s for pt: %f %f %d", screen.x, screen.y, i);
                vertices[vert_index] = {{screen.x, screen.y}, seg.color} ;
   ...
   // int indicest[3] = {1, 2, 3};
        SDL_RenderGeometry(renderer->get(), NULL, vertices , total_vertices, indices.data(), indices.size());
