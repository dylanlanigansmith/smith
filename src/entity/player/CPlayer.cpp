...
void CPlayer::OnRenderEnd()
{
    ...
    double pitchSpeed = frameTime * 3.0; //the constant value is in radians/second
     ...
     if(m_move.w)
    {
      if(ILevelSystem->GetMapAt(m_vecPosition.x + Camera().m_vecDir.x * moveSpeed, int(m_vecPosition.y)) == false) m_vecPosition.x += Camera().m_vecDir.x * moveSpeed;
      if(ILevelSystem->GetMapAt(int(m_vecPosition.x), int(m_vecPosition.y + Camera().m_vecDir.y * moveSpeed)) == false) m_vecPosition.y += Camera().m_vecDir.y * moveSpeed;
    }
    //move backwards if no wall behind you
    if(m_move.s)
    {
      if(ILevelSystem->GetMapAt(int(m_vecPosition.x - Camera().m_vecDir.x * moveSpeed), int(m_vecPosition.y)) == false) m_vecPosition.x -= Camera().m_vecDir.x * moveSpeed;
      if(ILevelSystem->GetMapAt(int(m_vecPosition.x), int(m_vecPosition.y - Camera().m_vecDir.y * moveSpeed)) == false) m_vecPosition.y -= Camera().m_vecDir.y * moveSpeed;
    }
    //rotate to the right
    if(m_move.d)
     ...
     if(IInputSystem->IsKeyDown(SDL_SCANCODE_UP))
    {
      ...
      if(m_camera.m_flPitch > 200) m_camera.m_flPitch = 200;
    }
    ...
    if(m_camera.m_flPitch > 0) m_camera.m_flPitch = std::max<double>(0, m_camera.m_flPitch - 100 * moveSpeed);
    if(m_camera.m_flPitch < 0) m_camera.m_flPitch = std::min<double>(0, m_camera.m_flPitch + 100 * moveSpeed);
    if( m_vecPosition.z > 0)  m_vecPosition.z = std::max<double>(0,  m_vecPosition.z - 100 * moveSpeed);
    if( m_vecPosition.z < 0)  m_vecPosition.z = std::min<double>(0,  m_vecPosition.z + 100 * moveSpeed);
}
