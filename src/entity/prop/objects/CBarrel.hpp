...
void CBarrel::OnCreate()
{
    ...
    this->m_hTexture = ITextureSystem->LoadTexture("barrel.png");
...
void CBarrel::Render(CRenderer *renderer)
{
    ...
    int vMoveScreen = int(vMove / transform.y) + camera->m_flPitch + (camera->m_vecPosition.z / transform.y);

    int renderHeight = std::abs(int(SCREEN_HEIGHT / (transform.y))) / vDiv; // using transform.y vs real distance prevents fisheye
     // calculate width of the sprite
    int renderWidth = abs(int(SCREEN_HEIGHT / (transform.y))) / uDiv;
